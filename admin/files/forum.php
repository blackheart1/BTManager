...
include_once 'include/class.bbcode.php';
            ...
            break;
                ...
                switch ($mode)
                {
                    ...
                    case 'takeforumgruop':
                        ...
                        {
                        //die(print_r($user_id) . print_r($username));
            ...
            switch ($action)
            {
                ...
                if($flood_intervals >= 0){
                    ...
                    set_config("flood_intervals",$flood_intervals);
                        $sqlfields[] = $flood_intervals;
                        $sqlvalues[] = "flood_intervals";
                    }
                    else
                        $errors[] = "Topics Perpage not Numeric";
                    }
                else
                    $errors[] = "flood_intervals";
                ...
                if($img_link_width >= 0){
                    ...
                    set_config("img_link_width",$img_link_width);
                        $sqlfields[] = $img_link_width;
                        $sqlvalues[] = "img_link_width";
                    }
                    else
                        $errors[] = "Topics Perpage not Numeric";
                }
                else
                    $errors[] = "img_link_width";

                if($img_link_height >= 0){
                    if(is_numeric($img_link_height)){
                    set_config("img_link_height",$img_link_height);
                        $sqlfields[] = $img_link_height;
                        $sqlvalues[] = "img_link_height";
                    }
                    else
                        $errors[] = "Topics Perpage not Numeric";
                }
                ...
                if($max_post_font_size >= 0){
                    ...
                    set_config("max_post_font_size",$max_post_font_size);
                        $sqlfields[] = $max_post_font_size;
                        $sqlvalues[] = "max_post_font_size";
                    }
                    else
                        $errors[] = "Topics Perpage not Numeric";
                }
                else
                    $errors[] = "max_post_font_size";

                if($max_attachments >= 0){
                    if(is_numeric($max_attachments)){
                    set_config("max_attachments",$max_attachments);
                        $sqlfields[] = $max_attachments;
                        $sqlvalues[] = "max_attachments";
                    }
                    else
                        $errors[] = "Topics Perpage not Numeric";
                }
                else
                    $errors[] = "max_attachments";

                if($min_post_chars >= 0){
                    if(is_numeric($min_post_chars)){
                    set_config("min_post_chars",$min_post_chars);
                        $sqlfields[] = $min_post_chars;
                        $sqlvalues[] = "min_post_chars";
                    }
                    else
                        $errors[] = "Topics Perpage not Numeric";
                }
                ...
                if($max_poll_options >= 0){
                    ...
                    set_config("max_poll_options",$max_poll_options);
                        $sqlfields[] = $max_poll_options;
                        $sqlvalues[] = "max_poll_options";
                    }
                    else
                        $errors[] = "Topics Perpage not Numeric";
                }
                ...
                if($hot_threshold >= 0){
                    ...
                    set_config("hot_threshold",$hot_threshold);
                        $sqlfields[] = $hot_threshold;
                        $sqlvalues[] = "hot_threshold";
                    }
                    else
                        $errors[] = "Topics Perpage not Numeric";
                }
                ...
                if($max_post_urls >= 0){
                    ...
                    set_config("max_post_urls",$max_post_urls);
                        $sqlfields[] = $max_post_urls;
                        $sqlvalues[] = "max_post_urls";
                    }
                    else
                        $errors[] = "Topics Perpage not Numeric";
                }
                ...
                if($max_post_smilies >= 0){
                    ...
                    set_config("max_post_smilies",$max_post_smilies);
                        $sqlfields[] = $max_post_smilies;
                        $sqlvalues[] = "max_post_smilies";
                    }
                    else
                        $errors[] = "Topics Perpage not Numeric";
                }
                ...
                if($max_quote_depth >= 0){
                    if(is_numeric($max_quote_depth)){
                        $sqlfields[] = $max_quote_depth;
                    set_config("max_quote_depth",$max_quote_depth);
                        $sqlvalues[] = "max_quote_depth";
                    }
                    else
                        $errors[] = "Topics Perpage not Numeric";
                }
                ...
                if($queue_trigger_posts >= 0){
                    ...
                    set_config("queue_trigger_posts",(($queue_trigger_posts == 'false')? 0 : 1));
                        $sqlfields[] = $queue_trigger_posts;
                        $sqlvalues[] = "queue_trigger_posts";
                    }
                    else
                        $errors[] = "Topics Perpage not Numeric";
                }
        ...
        switch ($action)
        {
            ...
            case 'move_down':
                ...
                {
                logerror(sprintf($user->lang['LOG_FORUM_' . strtoupper($action)], $row['forum_name'], $move_forum_name),'admin');
                }
            ...
            case 'sync_forum':
                ...
                logerror(sprintf($user->lang['LOG_FORUM_SYNC'], $row['forum_name']),'admin');
...
function get_groups()
{
    ...
    $template->assign_vars(array(
                    ...
                    'ICON_DELETE'             => $user->img('icon_delete', 'DELETE'),
                    'ICON_DELETE_DISABLED'    => $user->img('icon_delete_disabled', 'DELETE_DISABLED'),
                    'ICON_SYNC'               => $user->img('icon_resync', 'RESYNC'),
                    'ICON_SYNC_DISABLED'      => $user->img('icon_resync_disabled', 'RESYNC'),
    ));
