...
** File files/levels.php 2018-04-14 10:52:00 Thor
**
** CHANGES
**
...
//die($action);
        ...
        if ($group_id)
        {
            ...
            /*if ($user->group != USER_FOUNDER && $group_row['group_founder_manage'])
            {
        ...
        switch ($action)
        {
            ...
            case 'edit_site_prem':
                    ...
                    elseif($do == 'new_perm')
                    {
                        ...
                        if($take_add)
                        {
                            ...
                            $sql = 'INSERT INTO `' . $db_prefix . '_level_privlages` (
                                            `id` ,
                                            `acc_point` ,
                                            `descr`
                                            )
            ...
            case 'list':
                ...
                $sql = 'SELECT u.id, u.username, u.clean_username, u.user_colour, MICROSECOND(u.regdate), u.user_posts, u.can_do, ug.group_leader, ug.user_pending
                    ...
                    ORDER BY ug.group_leader DESC, ug.user_pending ASC, u.clean_username LIMIT " . $start . "," .$config['topics_per_page'];
                    //die($sql);
                $result = $db->sql_query($sql) or mysql_error();
        ...
        foreach ($cached_group_data as $type => $row_ary)
        {
            ...
            foreach ($row_ary as $group_id => $row)
            {
                ...
                $template->assign_block_vars('groups', array(
                    ...
                    'U_DELETE'      => (checkaccess("a_edit_level")) ? "{$u_action}&amp;action=delete&amp;g=$group_id" : '',
