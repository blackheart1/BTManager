...
* @version $Id: 3.0.0 mesage_parser.php  2014-02-02 00:22:48 joeroberts $
...
class bbcode_firstpass extends bbcode
{
	...
	*/
	function bbcode_init()
	{
		...
		$this->bbcodes = array(
			...
			'attachment'	=> array('bbcode_id' => 12,	'regexp' => array('#\[attachment=([0-900]+)\](.*?)\[/attachment\]#ise' => "\$this->bbcode_attachment('\$1', '\$2')")),
	...
	*/
	function bbcode_img($in)
	{
		...
		// Checking urls
		if (!preg_match("#(?i)\b((?:[a-z][\w-]+:(?:/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'\".,<>?������]))#iS", $in) && !preg_match('#^' . get_preg_expression('www_url') . '$#i', $in))
		{
			return '[img]' . $in . '[/img]';
		}
